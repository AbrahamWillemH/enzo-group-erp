...
<body class="flex flex-col items-center">
    <div class="flex flex-col items-center">
        ...
        <form method="POST" action="{{ route('logout') }}">
            @csrf
            <button type="submit" class="bg-decline text-white px-5 py-1 rounded-lg hover:bg-[#fff] hover:text-decline border hover:border-decline">
                Logout
            </button>
        </form>
    </div>
                        ...
                        <tr>
                            ...
                            <td class="flex flex-row gap-5">
                                <form action="{{ route('orders.approve', $order->id) }}" method="POST">
                                    @csrf
                                    <button type="submit" class="bg-accept text-white px-5 py-1 rounded-lg hover:bg-[#fff] hover:text-accept border hover:border-accept">
                                        Approve
                                    </button>
                                </form>
                            ...
                            @if ($order->progress != 'Selesai')
                                <td class="">
                                    <form action="{{ route('orders.updateProgress', $order->id) }}" method="POST" id="updateProgressForm{{ $order->id }}">
                                        @csrf
                                        <button type="submit" onclick="event.preventDefault(); confirmUpdate({{ $order->id }});" class="bg-accept text-white px-5 py-1 rounded-lg hover:bg-[#fff] hover:text-accept border hover:border-accept">
                                            Update Progress
                                        </button>
                                    </form>
                                </td>
